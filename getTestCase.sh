...
curl -s $(cat ${urlPath}) | egrep -A100 "(入力|出力)例" | sed -e "s/[\r\n]//g" | sed 's/\&amp;/&/g; s/\&lt;/</g; s/\&gt;/>/g; s/\&quot;/"/g; s/\&#39;/'"'"'/g' > inout

# count number of input-output pair
inoutNum=`cat inout | grep 入力例 | grep -c ^`

for i in `seq 1 $inoutNum`;
do
	# from inout, generate ${i}in and ${i}out
	cat inout | sed -n "/入力例 $i/,/<\/pre/p" | sed -e 's/.*pre>//g; $d' > ${i}"in"
	cat inout | sed -n "/出力例 $i/,/<\/pre/p" | sed -e 's/.*pre>//g; $d' > ${i}out
